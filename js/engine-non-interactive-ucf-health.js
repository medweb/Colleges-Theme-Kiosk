jQuery( document ).ready(function($) {
    ...
    const autoscroll_to_pages       = ['/marketing-kiosk/horizontal-news-ucf-health/'];  // after timeout, autoscroll these pages (in the order specified)
    const autoscroll_within_pages   = ['/marketing-kiosk/horizontal-news-ucf-health/']; // if we are on these pages, it will first autoscroll inner elements until the end, then go to the next autoscroll page.
    ...
    function autoslide_items(){
        ...
        var article_desired = get_article_array_number($('section.container article:visible').data('article-desired'));
        if (article_desired <= 1) {
